...
if TYPE_CHECKING:
    ...
    from llmtuner.hparams import DiffusionArguments, ModelArguments
...
def load_model_and_tokenizer(
    ...
    if stage == "sft":
        ...
        model.denoise_model = (
            prepare_model_for_training(model=model.denoise_model, finetuning_args=finetuning_args)
            if is_trainable
            else model.denoise_model
        )
        checkpoint_dir = model_args.checkpoint_dir
        model_args.checkpoint_dir = None
        model.denoise_model = init_adapter(
            model.denoise_model, model_args, finetuning_args, is_trainable, is_mergeable
        )
